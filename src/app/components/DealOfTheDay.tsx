...
function CountdownTimer({ targetDate }: { targetDate: Date }) {
  ...
  return (
    <div className="flex gap-1.5 sm:gap-2 md:gap-3 justify-center">
      {Object.entries(timeLeft).map(([unit, value]) => (
        <div key={unit} className="text-center">
          <div className="bg-black/80 backdrop-blur-md rounded-xl px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-3 min-w-[45px] sm:min-w-[60px] md:min-w-[70px]">
            <span className="text-xl sm:text-2xl md:text-3xl font-black text-white">
              {String(value).padStart(2, "0")}
            </span>
          </div>
          <p className="text-[8px] sm:text-xs text-gray-500 mt-0.5 sm:mt-1 capitalize">{unit}</p>
        </div>
      ))}
    </div>
  );
...
export default function DealOfTheDay() {
  ...
  if (loading) {
    return (
      <section className="bg-linear-to-r from-red-50 to-orange-50 py-10 sm:py-16 md:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="animate-pulse">
            <div className="h-6 sm:h-8 bg-gray-200 rounded w-36 sm:w-48 mx-auto mb-3 sm:mb-4" />
            <div className="h-8 sm:h-12 bg-gray-200 rounded w-64 sm:w-96 mx-auto mb-6 sm:mb-8" />
  ...
  return (
    <section className="bg-linear-to-r from-red-50 via-orange-50 to-amber-50 py-10 sm:py-16 md:py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <motion.div {...fadeUp(0)} className="text-center mb-6 sm:mb-8 md:mb-10">
          <span className="inline-flex items-center gap-1.5 sm:gap-2 bg-red-500 text-white text-[10px] sm:text-sm font-bold px-3 sm:px-4 py-1 sm:py-1.5 rounded-full shadow-lg">
            <Zap size={12} className="sm:w-3.5 sm:h-3.5" /> LIMITED TIME OFFER
          </span>
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black mt-3 sm:mt-4 text-gray-900">
            Deal of the <span className="text-pink-600">Day</span>
          </h2>
          <p className="text-sm sm:text-base md:text-lg text-gray-600 mt-1.5 sm:mt-2 md:mt-3">Grab it before it's gone! ⏰</p>
        </motion.div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 lg:gap-12 items-center">
          ...
          >
            <div className="relative h-64 sm:h-80 md:h-96 rounded-2xl overflow-hidden shadow-2xl">
              ...
              />
              {/* Discount Badge */}
              <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-red-600 text-white text-xl sm:text-2xl font-black px-3 sm:px-4 py-1.5 sm:py-2 rounded-full shadow-lg rotate-[-5deg]">
                -{deal.price > 0
                  ? Math.round(((deal.price - deal.dealPrice) / deal.price) * 100)
                  : 0}
                %
          ...
          {/* Product Details */}
          <motion.div {...fadeUp(0.2)} className="w-full lg:w-1/2 space-y-4 sm:space-y-5 md:space-y-6">
            <div>
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
                <span className="bg-green-100 text-green-700 text-[10px] sm:text-xs font-bold px-2 py-0.5 sm:py-1 rounded-full">
                  ⚡ Hot Deal
                </span>
                <span className="bg-orange-100 text-orange-700 text-[10px] sm:text-xs font-bold px-2 py-0.5 sm:py-1 rounded-full">
                  {deal.stock} left in stock
                </span>
              </div>
            ...
            <div className="flex flex-wrap items-baseline gap-2 sm:gap-3">
              ...
              <span className="bg-red-100 text-red-700 text-xs sm:text-sm font-bold px-2 py-0.5 sm:py-1 rounded-full">
                Save ₹{(
                  Number(deal.price ?? 0) -
                  Number(deal.dealPrice ?? 0)
                ).toLocaleString("en-IN")}
            ...
            {/* Countdown Timer */}
            <div className="bg-linear-to-r from-red-600 to-orange-600 rounded-2xl p-4 sm:p-5 text-center shadow-xl">
              <p className="text-white font-bold mb-2 sm:mb-3 flex items-center justify-center gap-2 text-sm sm:text-base">
                <Clock size={16} className="sm:w-[18px] sm:h-[18px]" /> Hurry! Offer ends in:
              </p>
              <CountdownTimer targetDate={targetDate} />
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2 sm:pt-4">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={handleAddToCart}
                className="flex-1 bg-linear-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white font-bold py-3 sm:py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all text-sm sm:text-base"
              >
                <ShoppingCart size={16} className="sm:w-5 sm:h-5" />
                Add to Cart – ₹{deal.dealPrice}
              </motion.button>
              <Link href={`/products/${deal._id}`} className="flex-1">
                <button className="w-full border-2 border-gray-300 hover:border-pink-600 text-gray-700 hover:text-pink-600 font-bold py-3 sm:py-4 rounded-xl transition-all text-sm sm:text-base">
                  View Details
                </button>
              </Link>
            </div>
            ...
            {/* Trust Badges */}
            <div className="flex flex-wrap justify-center gap-2 sm:gap-4 pt-2 sm:pt-4 text-[10px] sm:text-xs text-gray-500">
              <span className="flex items-center gap-1">✅ Free Shipping</span>
              <span className="flex items-center gap-1">🔄 7-Day Returns</span>
              <span className="flex items-center gap-1">🔒 Secure Checkout</span>
            </div>
        ...
        {/* Bumper Note */}
        <motion.p {...fadeUp(0.3)} className="text-center text-gray-400 text-xs sm:text-sm mt-6 sm:mt-8 md:mt-10">
          *Limited stock available. Offer valid while supplies last.
        </motion.p>
      </div>
    </section>
